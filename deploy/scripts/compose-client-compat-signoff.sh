...
ensure_compose_stack_running() {
  ...
  grep -qx "forgeframe" <<<"$running" || fail "forgeframe container is not running. Run bash deploy/scripts/compose-smoke.sh first."
  grep -qx "postgres" <<<"$running" || fail "postgres container is not running. Run bash deploy/scripts/compose-smoke.sh first."
}
