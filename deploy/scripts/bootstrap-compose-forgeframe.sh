...
ENV_EXAMPLE="$ROOT_DIR/deploy/docker/.env.compose.example"
...
docker compose -f "$COMPOSE_FILE" run --rm forgeframe python /app/deploy/scripts/apply-storage-migrations.py >/tmp/forgeframe-storage-migrations.json
...
bash "$ROOT_DIR/deploy/scripts/compose-smoke.sh"
...
if [[ "${FORGEFRAME_BOOTSTRAP_SKIP_RESTORE_SMOKE:-0}" == "1" ]]; then
  ...
  bash "$ROOT_DIR/deploy/scripts/compose-backup-restore-smoke.sh"
fi
