...
start_local_api_without_systemd() {
  ...
  FORGEFRAME_ENV_FILE="$ENV_FILE" "$ROOT_DIR/deploy/scripts/start-forgeframe.sh" >"$log_file" 2>&1 <"$FORGEFRAME_NULL_DEVICE" &
  LOCAL_API_PID="$!"
...
"$ROOT_DIR/deploy/scripts/install-forgeframe.sh" "${INSTALL_ARGS[@]}"
...
if [[ "$SKIP_SYSTEMCTL" != "1" ]]; then
  ...
  if (( ${#public_contract_errors[@]} == 0 )); then
    ...
    bash "$ROOT_DIR/deploy/scripts/renew-certificates.sh"
...
if (( ${#public_contract_errors[@]} == 0 )); then
  bash "$ROOT_DIR/deploy/scripts/host-smoke.sh"
else
