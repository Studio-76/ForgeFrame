...
if [[ "${FORGEFRAME_LIMITED_STDLIB_RUNTIME:-0}" == "1" ]]; then
  ...
  exec "$PYTHON_BIN" "$ROOT_DIR/deploy/scripts/serve-limited-forgeframe.py"
fi

for ((attempt = 1; attempt <= MIGRATION_ATTEMPTS; attempt++)); do
  if migration_report="$("$PYTHON_BIN" "$ROOT_DIR/deploy/scripts/apply-storage-migrations.py" 2>&1)"; then
