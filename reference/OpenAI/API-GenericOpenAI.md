...
Generischer Adapter fuer OpenAI-kompatible Endpunkte. In code-proxy ist das der Provider `generic-openai`. In hermes-agent entsprechen viele Direct-/Aggregator-Provider ebenfalls diesem Muster.
...
```json
{
  "model": "provider/model-or-native-id",
...
Antwort als Server-Sent Events:
...
`api_generic.go` inferiert diese Bases:
...
Achtung: code-proxy haengt immer `/v1/chat/completions` an. Die Base URL muss deshalb so gewaehlt werden, dass dieser Pfad stimmt.
...
async def call_openai_compatible(base_url, token, payload, stream=False):
    url = base_url.rstrip('/') + '/v1/chat/completions'
    headers = {'Authorization': f'Bearer {token}', 'Content-Type': 'application/json'}
    if stream:
        async with httpx.AsyncClient(timeout=None) as client:
            async with client.stream('POST', url, headers=headers, json={**payload, 'stream': True}) as r:
                r.raise_for_status()
                async for line in r.aiter_lines():
                    if line.startswith('data: '):
                        yield line[6:]
    else:
        r = httpx.post(url, headers=headers, json={**payload, 'stream': False}, timeout=120)
        r.raise_for_status()
        return r.json()
```
