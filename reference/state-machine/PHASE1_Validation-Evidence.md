...
| 4 | `decide_approval` does not clear `run.current_approval_link_id`. | **Accepted as intended behavior.** DECISION_SPEC5-Finding4.md: retained link is an audit trail, not a control signal. No code change required. | No longer blocks Phase 1c — decision issued. `approval_link_mismatch` category must remain active after authority transfer. |
| 5 | `request_cancel` can cancel a `waiting_on_approval` run without closing the approval link. | **Accepted as intended behavior.** DECISION_SPEC5-Finding5.md: retained open link correctly signals the approval was never formally decided. No code change required. | No longer blocks Phase 1c — decision issued. `approval_link_mismatch` category must remain active after authority transfer. |
