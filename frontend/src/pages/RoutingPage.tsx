...
function nextStepLabel(status: "ready" | "partial" | "degraded" | "blocked"): string {
  switch (status) {
    case "blocked":
...
function nextStepTone(status: "ready" | "partial" | "degraded" | "blocked"): StatusTone {
  switch (status) {
    case "blocked":
      return "danger";
    case "degraded":
    ...
    <section className="fg-page">
      ...
      {snapshot && canReadRouting ? (
        <>
          ...
          {/* ── Status Hero ── */}
          <section className="ff-status-hero" aria-label="Routing status">
            <div className="ff-status-hero-top">
              <div>
                <h3 className="ff-status-hero-label">
                  {titleCase(liveStatusKey)} — {policies.length >= 2 ? "Simple + non-simple" : `${policies.length} policy record(s)`}
                </h3>
                <p className="ff-status-hero-line">
                  {blockers.length === 0
                    ? "Budget, circuits, and target readiness are not currently blocking routing."
                    : blockers[0]}
                </p>
              </div>
            ...
            {blockers.length > 0 ? (
              <div className="ff-next-step" data-tone={nextStepTone(liveStatusKey)}>
                <span className="ff-next-step-label">{nextStepLabel(liveStatusKey)}</span>
          ...
          <section className="ff-action-bar">
            <div className="ff-action-bar-header">
              <div className="ff-action-bar-copy">
                ...
                <p>Edit policy, run simulations, or navigate to related surfaces.</p>
              </div>
            ...
            <div className="ff-nav-links">
              ...
              <Link className="fg-nav-link" to={withInstanceScope(CONTROL_PLANE_ROUTES.dashboard, instanceId)}>Setup</Link>
            </div>
          </section>
          ...
          <div className="ff-operator-layout">
            <div className="ff-operator-main">
              ...
              <details
                ...
                open={policyOpen || policies.length === 0}
                onToggle={(event) => setPolicyOpen((event.target as HTMLDetailsElement).open)}
              >
                <summary>
                  <div className="ff-collapse-summary-text">
                    <h3>Policy editor</h3>
                    <p>{policies.length >= 2 ? "Simple and non-simple policies are configured." : `${policies.length} of 2 policies configured — expand to edit.`}</p>
                  </div>
              ...
              {/* ── Budget and Circuit Guardrails (collapsible) ── */}
              <details
                id="routing-budget-circuits"
                className="ff-collapse-section"
                open={budgetOpen}
                onToggle={(event) => setBudgetOpen((event.target as HTMLDetailsElement).open)}
              >
                <summary>
                  <div className="ff-collapse-summary-text">
                    <h3>Budget and circuit guardrails</h3>
                    <p>{blockers.length === 0 ? "No active budget or circuit blockers." : `${blockers.length} blocker(s): ${blockers[0]}`}</p>
                  </div>
                  <StatusBadge tone={toneForStatus(liveStatusKey)} status={liveStatusKey}>
                    {titleCase(liveStatusKey)}
                  </StatusBadge>
              ...
              {/* ── Dry-run Simulation (collapsible) ── */}
              <details
                id="routing-simulation"
                className="ff-collapse-section"
                open={simOpen}
                onToggle={(event) => setSimOpen((event.target as HTMLDetailsElement).open)}
              >
                <summary>
                  <div className="ff-collapse-summary-text">
                    <h3>Dry-run simulation</h3>
                    <p>{simulationResult ? `Last result: ${simulationSummary(simulationResult)}` : "Simulate a routing decision before it goes live."}</p>
                  </div>
              ...
              <details
                ...
                className="ff-collapse-section"
                open={decisionsOpen}
                onToggle={(event) => setDecisionsOpen((event.target as HTMLDetailsElement).open)}
              >
                <summary>
                  <div className="ff-collapse-summary-text">
                    <h3>Recent decisions</h3>
                    <p>{recentDecisions.length > 0 ? `${recentDecisions.length} decisions recorded. ${blockedDecisions.length} blocked.` : "No routing decisions are persisted yet."}</p>
                  </div>
