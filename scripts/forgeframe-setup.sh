...
setup_compose() {
  ...
  gum_spin "Starting Docker Compose..." \
    bash "$ROOT_DIR/scripts/bootstrap-compose-forgeframe.sh"
}
...
setup_host_native() {
  ...
  gum_spin "Running host-native installer..." \
    bash "$ROOT_DIR/scripts/install-forgeframe.sh" --guided
}
...
setup_limited() {
  ...
  gum_spin "Setting up limited exception..." \
    bash "$ROOT_DIR/scripts/install-forgeframe.sh" --allow-file-storage --skip-system-deps --skip-systemctl
}
...
if [[ "$MODE" != "dev" && "$NON_INTERACTIVE" != "1" ]]; then
  ...
  case "$MODE" in
    ...
    host-native)
      ...
      ;;
    limited)
      gum format -- "- Limited file/SQLite runtime: ./scripts/start-forgeframe.sh"
