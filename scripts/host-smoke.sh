...
curl "${CURL_ARGS[@]}" "$HEALTH_URL" >/dev/null || fail "Health endpoint did not become ready at $HEALTH_URL"
...
if "<html" not in payload and "<!doctype html" not in payload:
    raise SystemExit(1)
PY
curl "${CURL_ARGS[@]}" "$MODELS_URL" >/dev/null || fail "Runtime API is not reachable on the same public origin at $MODELS_URL"
