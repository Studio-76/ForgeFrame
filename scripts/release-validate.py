...
REPORT_PATH = Path(
    os.environ.get("FORGEFRAME_RELEASE_REPORT_PATH", str(DEFAULT_REPORT_PATH))
)
...
def _passed(
    step_id: str, details: str, *, command: list[str] | None = None
) -> StepResult:
    return StepResult(
        id=step_id, status="passed", source="internal", details=details, command=command
    )
...
def _blocked(
    step_id: str, details: str, *, source: str, command: list[str] | None = None
) -> StepResult:
    return StepResult(
        id=step_id, status="blocked", source=source, details=details, command=command
    )


def _run_command(
    step_id: str, command: list[str], *, cwd: Path | None = None
) -> StepResult:
    try:
...
def _gate(
    step_id: str, ok: bool, success_details: str, failure_details: str
) -> StepResult:
    if ok:
...
def _linux_shell_step(step_id: str, script_name: str) -> StepResult:
    ...
    if platform.system() != "Linux":
        return _blocked(
            step_id,
            "linux_host_required_for_this_release_gate",
            source="unsupported_host",
            command=command,
        )
    if shutil.which("bash") is None:
        return _blocked(
            step_id, "bash_not_available", source="external_dependency", command=command
        )
    return _run_command(step_id, command, cwd=ROOT_DIR)


def _python_driver_step(
    step_id: str, script_name: str, *, args: list[str] | None = None
) -> StepResult:
...
def _public_ingress_mode_step() -> StepResult:
    injected_defaults = {
        "FORGEFRAME_BOOTSTRAP_ADMIN_PASSWORD": os.environ.get(
            "FORGEFRAME_BOOTSTRAP_ADMIN_PASSWORD", "release-validate-placeholder"
        ),
        "FORGEFRAME_HARNESS_POSTGRES_URL": os.environ.get(
            "FORGEFRAME_HARNESS_POSTGRES_URL",
            "postgresql://forgeframe:test@127.0.0.1:5432/forgeframe",
        ),
        "FORGEFRAME_CONTROL_PLANE_POSTGRES_URL": os.environ.get(
            "FORGEFRAME_CONTROL_PLANE_POSTGRES_URL",
            "postgresql://forgeframe:test@127.0.0.1:5432/forgeframe",
        ),
        "FORGEFRAME_OBSERVABILITY_POSTGRES_URL": os.environ.get(
            "FORGEFRAME_OBSERVABILITY_POSTGRES_URL",
            "postgresql://forgeframe:test@127.0.0.1:5432/forgeframe",
        ),
        "FORGEFRAME_GOVERNANCE_POSTGRES_URL": os.environ.get(
            "FORGEFRAME_GOVERNANCE_POSTGRES_URL",
            "postgresql://forgeframe:test@127.0.0.1:5432/forgeframe",
        ),
        "FORGEFRAME_INSTANCES_POSTGRES_URL": os.environ.get(
    ...
    try:
        ...
        return _failed(
            ...
            f"ingress_status_unavailable:{type(exc).__name__}:{exc}",
        )
    finally:
...
def main() -> int:
    ...
    host_env_example = _read_text(
        ROOT_DIR / "deploy" / "env" / "forgeframe-host.env.example"
    )
    ...
    steps = [
        ...
        _gate(
            ...
            (
                ...
                and "FORGEFRAME_PUBLIC_TLS_ACME_EMAIL=replace-with-acme-email@example.invalid"
                in host_env_example
            ),
        ...
        _gate(
            ...
            "docker compose" not in bootstrap_script
            and "docker compose" not in install_script,
            "bootstrap_driver_is_host_native",
            "bootstrap_driver_still_depends_on_compose",
        ),
        _gate(
            "host_native_recovery_scripts",
            "docker compose" not in backup_script
            and "docker compose" not in restore_script,
        ...
        _gate(
            ...
            (
                ...
                and "Runtime API is not reachable on the same public origin"
                in host_smoke_script
            ),
        ...
        _gate(
            ...
            "${FORGEFRAME_PORT:-8000}" not in start_script
            and "${FORGEFRAME_APP_PORT:-8000}" not in bootstrap_script,
        ...
        _gate(
            ...
            "http://127.0.0.1" not in bootstrap_script
            and "/app/" not in bootstrap_script,
