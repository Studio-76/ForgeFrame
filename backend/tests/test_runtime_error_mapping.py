...
def test_runtime_routing_errors_map_to_public_contract_codes() -> None:
    budget_status, budget_code, _, budget_message, budget_extra = _provider_exception_to_http(RoutingBudgetExceededError("budget posture"))
    ...
    circuit_status, circuit_code, _, circuit_message, circuit_extra = _provider_exception_to_http(RoutingCircuitOpenError("circuits open"))
    ...
    dispatch_status, dispatch_code, _, dispatch_message, dispatch_extra = _provider_exception_to_http(RoutingNoCandidateError("no candidate"))
